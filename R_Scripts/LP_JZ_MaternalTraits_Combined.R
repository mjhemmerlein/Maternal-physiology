...
for (trait in traits) {
  ...
  # write.csv(DE_trait, file = paste0("LP_JZ_Combined_DE_trait_", trait, ".csv"), row.names = TRUE)
  # write.csv(DE_o2, file = paste0("LP_JZ_Combined_DE_O2_", trait, ".csv"), row.names = TRUE)
}

# Save summary table
write.csv(sig_counts, file = "LP_JZ_Combined_SIG_summary.csv", row.names = FALSE)
