...
# Filter to BW -------
LP_LZ_Sample_Info = LP_LZ_Sample_Info %>% filter(Strain == "BW")
BW = LP_LZ_Sample_Info$LZ_Seq_Name
...
Pman_rawreads_LP_LZ <- Pman_rawreads_LP_LZ[,-c(1:6)]
Pman_rawreads_LP_LZ <- subset(Pman_rawreads_LP_LZ, select = -c(RNA201216ZC_LZ089_S16_L001_fastp_pman_Halign_liberal.bam))
Pman_rawreads_LP_LZ <- Pman_rawreads_LP_LZ[,names(Pman_rawreads_LP_LZ) %in% BW]
Pman_readcounts_LP_LZ <- as.matrix(Pman_rawreads_LP_LZ)

dPman_0 <- DGEList(Pman_readcounts_LP_LZ)
dPman_0 <- calcNormFactors(dPman_0)
dim(dPman_0)

dPman_BW <- dPman_0[keep_LP_LZ,]
dim(dPman_BW)
plotMDS(dPman_BW, col = as.numeric(LP_LZ_Sample_Info$Strain), labels = LP_LZ_Sample_Info$Strain)
...
for (trait in traits) {
  ...
  write.csv(DE_trait, file = paste0("LP_LZ_BW_DE_trait_", trait, ".csv"), row.names = TRUE)
  write.csv(DE_ixn, file = paste0("LP_LZ_BW_DE_ixn_", trait, ".csv"), row.names = TRUE)
}

# Save summary table
write.csv(sig_counts, file = "LP_LZ_BW_SIG_summary.csv", row.names = FALSE)
...
# Filter to ME -------
LP_LZ_Sample_Info = LP_LZ_Sample_Info %>% filter(Strain == "ME")
ME = LP_LZ_Sample_Info$LZ_Seq_Name
...
Pman_rawreads_LP_LZ <- Pman_rawreads_LP_LZ[,-c(1:6)]
Pman_rawreads_LP_LZ <- subset(Pman_rawreads_LP_LZ, select = -c(RNA201216ZC_LZ089_S16_L001_fastp_pman_Halign_liberal.bam))
Pman_rawreads_LP_LZ <- Pman_rawreads_LP_LZ[,names(Pman_rawreads_LP_LZ) %in% ME]
Pman_readcounts_LP_LZ <- as.matrix(Pman_rawreads_LP_LZ)
...
dPman_ME <- dPman_0[keep_LP_LZ,]
dim(dPman_ME)
plotMDS(dPman_ME, col = as.numeric(LP_LZ_Sample_Info$Strain), labels = LP_LZ_Sample_Info$Strain)
...
colnames(dPman_ME) <- rownames(LP_LZ_Sample_Info)
...
for (trait in traits) {
  ...
  # Save individual trait results
  write.csv(DE_trait, file = paste0("LP_LZ_ME_DE_trait_", trait, ".csv"), row.names = TRUE)
  write.csv(DE_ixn, file = paste0("LP_LZ_ME_DE_ixn_", trait, ".csv"), row.names = TRUE)
}

# Save summary table
write.csv(sig_counts, file = "LP_LZ_ME_SIG_summary.csv", row.names = FALSE)
